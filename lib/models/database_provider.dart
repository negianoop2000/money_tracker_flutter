...
class DatabaseProvider with ChangeNotifier {
  ...
  DatabaseProvider([Database? database]) {
    _database = database; // Use provided database or real one if none is provided
  }
  ...
  Future<void> clearAllExpenses() async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.delete(eTable).then((_) {
        _expenses.clear();
        notifyListeners();

        // Reset the category entries and total amounts to 0
        for (var category in _categories) {
          updateCategory(category.title, 0, 0.0);
        }
      });
    });
  }
